...
def load_model_and_tokenizer():
    """Load trained BERT model correctly"""

    # Check if files are in current directory or a subfolder
    model_path = Path(".") if (Path("config.json").exists() and Path("model.safetensors").exists()) else Path("bert_news_classifier")

    try:
        if not model_path.exists() or not (model_path / "config.json").exists():
            st.error(f"Model files not found at {model_path.absolute()}!")
            return None, None

        # FORCE LOCAL FILE LOADING (IMPORTANT FIX)
        ...
        model.eval()  # 🔥 IMPORTANT for correct inference mode
...
def load_config():
    ...
    try:
        # Check current dir first, then subfolder
        config_path = Path("config.json") if Path("config.json").exists() else Path("bert_news_classifier/config.json")
...
def classify_news(text: str, model, tokenizer, config: Dict, device) -> Dict:
    """
    Stable inference function (FIXED VERSION)
    """
...
def main():
    ...
    if model is None or tokenizer is None:
        ...
        st.info("💡 Make sure the 'bert_news_classifier' folder exists in the same directory as this script.")
        return

    # Move to device only after confirming model exists
    ...
    with tab1:
        ...
        if clear_btn:
            user_text = "" # Clear the text area/input
            st.session_state.selected_text = "" # Clear selected example
        ...
        if classify_btn:
            if not user_text.strip():
                ...
                with st.spinner("🔄 Classifying..."):
                    result = classify_news(user_text, model, tokenizer, config, device)
